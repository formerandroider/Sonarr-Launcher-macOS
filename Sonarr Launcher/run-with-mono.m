...
int runAssemblyPathWithMono(NSString *appName, NSString *assemblyPath, int major, int minor) {
    NSFileManager *fileManager = [NSFileManager defaultManager];
...
int runAssemblyWithMono(NSString *appName, NSString *assembly, int major, int minor) {
    NSString *entryFolder = [[NSBundle mainBundle] executablePath];
    if (D()) NSLog(@"entryFolder: %@", entryFolder);

    NSString *assemblyPath = [[NSBundle mainBundle] pathForAuxiliaryExecutable:assembly];
    if (D()) NSLog(@"assemblyPath: %@", assemblyPath);
    
    return runAssemblyPathWithMono(appName, assemblyPath, major, minor);
}
